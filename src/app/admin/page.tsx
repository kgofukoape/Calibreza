...
export default function AdminOverviewPage() {
  ...
  const loadData = async () => {
    const [listingsRes, dealersRes, pendingRes, recentListingsRes, recentDealersRes] = await Promise.all([
        ...
        <div className="p-8 space-y-8">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-5 gap-4">
            {[
              ...
              { label: 'Pending', value: stats.pendingApplications, icon: '⏳', color: 'text-[#F59E0B]', border: 'border-[#F59E0B]/20' },
            ].map((stat) => (
                ...
                {pendingDealers.map((dealer) => (
                  <div key={dealer.id} className="px-6 py-4 flex items-center gap-4">
                    ...
                    <div className="flex items-center gap-2 flex-shrink-0">
                      <Link href="/admin/dealers" className="text-[10px] font-black uppercase tracking-widest text-[#4CC9F0] border border-[#4CC9F0]/30 px-3 py-1.5 rounded-sm hover:bg-[#4CC9F0]/10 transition-all">
                        View
                      </Link>
                      <button
          ...
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
            ...
            <div className="bg-[#0D1420] border border-white/5 rounded-sm overflow-hidden">
              <div className="px-6 py-4 border-b border-white/5 flex items-center justify-between">
                ...
                <Link href="/admin/listings" className="text-[10px] font-black uppercase tracking-widest text-white/40 hover:text-white transition-colors">View All →</Link>
              </div>
              <div className="divide-y divide-white/5">
                  ...
                  <div key={listing.id} className="px-6 py-3 flex items-center gap-4">
                    ...
                    <div className="flex items-center gap-2 flex-shrink-0">
                      <span className={`text-[9px] font-black uppercase tracking-widest px-2 py-1 rounded-sm ${
                        listing.status === 'active' ? 'bg-[#10B981]/10 text-[#10B981] border border-[#10B981]/20' : 'bg-white/5 text-white/40 border border-white/10'
                      }`}>
                        {listing.status}
                      </span>
            ...
            <div className="bg-[#0D1420] border border-white/5 rounded-sm overflow-hidden">
              <div className="px-6 py-4 border-b border-white/5 flex items-center justify-between">
                ...
                <Link href="/admin/dealers" className="text-[10px] font-black uppercase tracking-widest text-white/40 hover:text-white transition-colors">View All →</Link>
              </div>
              <div className="divide-y divide-white/5">
                {recentDealers.map((dealer) => (
                  <div key={dealer.id} className="px-6 py-3 flex items-center gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[10px] text-white/40 uppercase tracking-wider">{dealer.city}, {dealer.province}</p>
                    </div>
                    <span className={`text-[9px] font-black uppercase tracking-widest px-2 py-1 rounded-sm flex-shrink-0 ${
                      dealer.status === 'approved' ? 'bg-[#10B981]/10 text-[#10B981] border border-[#10B981]/20' :
                      dealer.status === 'pending' ? 'bg-[#F59E0B]/10 text-[#F59E0B] border border-[#F59E0B]/20' :
