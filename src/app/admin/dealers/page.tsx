...
const NAV = [
  ...
  { href: '/admin/dealers',   icon: '🏪', label: 'Dealers', active: true },
...
export default function AdminDealersPage() {
  ...
  useEffect(() => {
    // Auth check using localStorage — same as all other admin pages
            ...
            {NAV.map(item => (
              <li key={item.href}>
                <Link href={item.href} className={`flex items-center gap-3 px-3 py-2.5 rounded-sm font-black text-[11px] uppercase tracking-widest transition-all ${
                  ...
                  {item.href === '/admin/dealers' && statusCounts.pending > 0 && (
                    <span className="bg-[#F59E0B] text-black text-[9px] font-black px-1.5 py-0.5 rounded-full">{statusCounts.pending}</span>
                  )}
                </Link>
              </li>
            ))}
      ...
      <main className="flex-1 ml-[260px] overflow-y-auto">
        <header className="bg-[#0D1420] border-b border-white/5 px-8 py-5 sticky top-0 z-40">
          ...
          <p className="text-white/40 text-xs mt-0.5 uppercase tracking-widest font-bold">
            {dealers.length} total · {statusCounts.pending} pending
          </p>
        </header>
              ...
              <div className="flex gap-1">
                {STATUS_FILTERS.map(f => (
                  <button key={f} onClick={() => setStatusFilter(f)}
                    className={`flex-1 py-1.5 rounded-sm text-[9px] font-black uppercase tracking-widest transition-all ${statusFilter === f ? 'bg-[#E63946] text-white' : 'bg-white/5 text-white/40 hover:bg-white/10'}`}>
            ...
            <div className="flex-1 overflow-y-auto divide-y divide-white/5">
              {filtered.map(dealer => (
                <button key={dealer.id} onClick={() => setSelectedDealer(dealer)}
                  className={`w-full text-left px-4 py-4 hover:bg-white/5 transition-all ${selectedDealer?.id === dealer.id ? 'bg-[#E63946]/5 border-l-2 border-[#E63946]' : ''}`}>
                  <div className="flex items-center gap-3">
                    ...
                    }`}>{dealer.business_name?.charAt(0) || 'D'}</div>
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-black text-white truncate">{dealer.business_name}</p>
                      <p className="text-[10px] text-white/40 uppercase tracking-wider truncate">{dealer.city}, {dealer.province}</p>
                    </div>
              ...
              {filtered.length === 0 && <div className="p-8 text-center text-white/30 text-sm">No dealers found</div>}
            </div>
          </div>
          ...
          <div className="flex-1 overflow-y-auto">
            {selectedDealer ? (
                ...
                <div className="flex items-start justify-between">
                  <div className="flex items-center gap-4">
                    <div className={`w-14 h-14 rounded-sm flex items-center justify-center font-black text-2xl ${
                      ...
                      'bg-[#E63946]/10 text-[#E63946]'
                    }`}>{selectedDealer.business_name?.charAt(0) || 'D'}</div>
                    <div>
                      <h2 style={{ fontFamily: "'Barlow Condensed', sans-serif" }} className="text-2xl font-black uppercase text-white">{selectedDealer.business_name}</h2>
                      <p className="text-white/40 text-xs uppercase tracking-widest">Applied {formatDate(selectedDealer.created_at)}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    <Link href={`/dealers/${selectedDealer.slug}`} target="_blank"
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="bg-[#0D1420] border border-white/5 rounded-sm p-4">
                    ...
                    <div className="flex gap-2 flex-wrap">
                      {TIERS.map(tier => (
                        <button key={tier} onClick={() => handleTierChange(selectedDealer.id, tier)}
              ...
              <div className="h-full flex items-center justify-center flex-col gap-4 text-white/20">
                ...
                <p className="text-sm uppercase tracking-widest font-bold">Select a dealer to view details</p>
              </div>
            )}
          </div>
        </div>
