...
export default function DashboardPage() {
  ...
  const loadUserData = async () => {
    ...
    try {
      ...
      // Redirect clubs/ranges to their dashboard
      ...
      // Redirect services to their dashboard
  ...
  const toggleUnderOffer = async (listingId: string, currentStatus: string) => {
    ...
    try {
      ...
      if (data && !data.success) { alert(`Failed: ${data.error}`); return; }
  ...
  const getStatusBadge = (status: string) => {
    if (status === 'sold') return <span className="px-2 py-1 rounded-sm text-[11px] font-bold uppercase bg-red-500/20 text-red-400 border border-red-500/30">Sold</span>;
    if (status === 'under_offer') return <span className="px-2 py-1 rounded-sm text-[11px] font-bold uppercase bg-[#C9922A]/20 text-[#C9922A] border border-[#C9922A]/30">Under Offer</span>;
    ...
    <div className="min-h-screen bg-[#0D0F13] flex">
      <aside className="w-64 bg-[#191C23] border-r border-white/5 flex flex-col">
        <div className="p-6 border-b border-white/5">
          <Link href="/" className="block">
            ...
            <span className="block text-[10px] uppercase tracking-[0.3em] font-bold text-[#8A8E99] mt-1">Dashboard</span>
          </Link>
        </div>
        ...
        <div className="p-6 border-b border-white/5">
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              <div className="font-bold text-[14px] text-[#F0EDE8] truncate">{user?.full_name || user?.email?.split('@')[0] || 'User'}</div>
              <div className="text-[11px] text-[#8A8E99] truncate">{user?.email}</div>
            </div>
          </div>
        </div>
      ...
      <main className="flex-1 overflow-auto">
        <div className="bg-[#191C23] border-b border-white/5 p-8 flex items-center justify-between">
          <div>
            <h1 style={{fontFamily:"'Barlow Condensed', sans-serif"}} className="text-4xl font-extrabold uppercase text-[#F0EDE8] mb-2">Dashboard Overview</h1>
            <p className="text-[14px] text-[#8A8E99]">Welcome back, {user?.full_name || user?.email?.split('@')[0] || 'User'}</p>
          </div>
          <a href="/sell" className="bg-[#C9922A] text-black font-bold px-6 py-3 rounded-sm text-[14px] uppercase hover:brightness-110 transition-all shadow-lg flex items-center gap-2">
            <span className="text-[18px]">+</span>Post Listing
          </a>
        </div>
        ...
        <div className="p-8">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
            <div className="bg-[#191C23] border border-white/5 rounded-md p-6">
              ...
              <div className="text-3xl font-bold text-[#F0EDE8]">{listings.filter(l => l.status === 'active').length}</div>
            </div>
            <div className="bg-[#191C23] border border-white/5 rounded-md p-6">
              <div className="text-[13px] text-[#8A8E99] mb-2">Total Views</div>
              <div className="text-3xl font-bold text-[#F0EDE8]">{listings.reduce((sum, l) => sum + (l.views_count || 0), 0)}</div>
            </div>
            <div className="bg-[#191C23] border border-white/5 rounded-md p-6">
              <div className="text-[13px] text-[#8A8E99] mb-2">Under Offer</div>
              <div className="text-3xl font-bold text-[#C9922A]">{listings.filter(l => l.status === 'under_offer').length}</div>
            </div>
            <div className="bg-[#191C23] border border-white/5 rounded-md p-6">
              <div className="text-[13px] text-[#8A8E99] mb-2">Sold</div>
              <div className="text-3xl font-bold text-red-400">{listings.filter(l => l.status === 'sold').length}</div>
            </div>
          </div>

          <div className="bg-[#191C23] border border-white/5 rounded-md p-6">
            <h2 style={{fontFamily:"'Barlow Condensed', sans-serif"}} className="text-2xl font-bold uppercase text-[#F0EDE8] mb-6">Your Recent Listings</h2>
            {listings.length === 0 ? (
                  ...
                  <div key={listing.id} className="bg-[#0D0F13] border border-white/5 rounded-md p-4 flex flex-col gap-4">
                    <div className="flex flex-col md:flex-row md:items-center gap-4">
                      ...
                      <div className="flex gap-2 flex-wrap">
                        <a href={`/dashboard/listings/edit/${listing.id}`} className="bg-[#C9922A] text-black font-bold px-3 py-2 rounded-sm text-[12px] uppercase hover:brightness-110 transition-all">EDIT</a>
                        <a href={`/listings/${listing.id}`} className="bg-transparent border border-white/20 text-[#F0EDE8] font-bold px-3 py-2 rounded-sm text-[12px] uppercase hover:bg-white/5 transition-all">VIEW</a>
                      </div>
                    </div>
                    <div className="flex gap-2 pt-3 border-t border-white/5">
                      <button onClick={() => toggleUnderOffer(listing.id, listing.status || 'active')}
                        className={`px-3 py-1.5 rounded-sm text-[11px] font-bold uppercase transition-all cursor-pointer ${listing.status === 'under_offer' ? 'bg-[#C9922A] text-black' : 'bg-transparent border border-[#C9922A]/30 text-[#C9922A] hover:bg-[#C9922A]/10'}`}>
                        {listing.status === 'under_offer' ? '✓ Under Offer' : 'Mark Under Offer'}
                      </button>
                      <button onClick={() => toggleSold(listing.id, listing.status || 'active')}
                        className={`px-3 py-1.5 rounded-sm text-[11px] font-bold uppercase transition-all cursor-pointer ${listing.status === 'sold' ? 'bg-red-500 text-white' : 'bg-transparent border border-red-500/30 text-red-400 hover:bg-red-500/10'}`}>
