...
export default function AccessoriesPage() {
  ...
  useEffect(() => {
    fetchListings();
  }, [currentPage, activeBrands, activeCategories, activeProvinces, activeConditions, activeMinPrice, activeMaxPrice, activeSellerTypes, sortBy]);
  ...
  const fetchListings = async () => {
    ...
    try {
      ...
      if (activeCategories.length > 0) query = query.in('accessory_category', activeCategories);
          ...
          <div className="bg-[#191C23] border border-white/5 rounded-md p-5 flex flex-col gap-6">
            ...
            <div className="flex flex-col gap-3">
              ...
              <div className="flex flex-col gap-2.5 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
                {[
                  'Optics & Scopes', 'Red Dot Sights', 'Holographic Sights', 'Reflex Sights', 
                  'Night Vision', 'Thermal Imaging', 'Rangefinders', 'Binoculars',
                  'Flashlights / Weapon Lights', 'Laser Sights', 'IR Illuminators',
                  'Slings', 'Bipods', 'Monopods', 'Shooting Sticks', 'Rests',
                  'Grips / Foregrips', 'Stocks', 'Buttstock Pads', 'Cheek Rests',
                  'Rails / Mounts', 'Picatinny Rails', 'Scope Rings', 'Scope Mounts',
                  'Suppressors / Silencers', 'Muzzle Brakes', 'Flash Hiders', 'Compensators',
                  'Cleaning Kits', 'Bore Snakes', 'Gun Oil / Lubricants', 'Cleaning Rods',
                  'Cases / Bags', 'Gun Cases', 'Range Bags', 'Backpacks',
                  'Targets', 'Target Stands', 'Shooting Mats', 'Chronographs',
                  'Hearing Protection', 'Eye Protection', 'Shooting Gloves',
                  'Patches / Morale Patches', 'Sling Swivels', 'QD Mounts',
                  'Other Accessories'
                ].map(cat => (
            ...
            <div className="flex flex-col gap-3 border-t border-white/5 pt-5">
              ...
              <div className="flex flex-col gap-2.5 max-h-[300px] overflow-y-auto pr-2 custom-scrollbar">
                {[
                  'Aimpoint', 'EOTech', 'Trijicon', 'Vortex', 'Leupold', 
                  'Holosun', 'SIG Sauer', 'Burris', 'Bushnell', 'Nikon', 
                  'Steiner', 'Swarovski', 'Zeiss', 'Schmidt & Bender', 'Nightforce',
                  'Magpul', 'BCM', 'Arisaka', 'Surefire', 'Streamlight', 
                  'Inforce', 'Olight', 'Crimson Trace', 'Viridian', 'LaserMax',
                  'Harris', 'Atlas', 'Caldwell', 'CTK Precision', 'Accu-Shot',
                  'Blue Force Gear', 'Vickers', 'Viking Tactics', 'Ferro Concepts',
                  'Peltor', 'Howard Leight', 'Walker\'s', 'Pro-Ears', 'Safariland'
                ].map(brand => (
        ...
        <div className="flex-1 flex flex-col gap-6">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-[#191C23] border border-white/5 rounded-md p-4">
            <span className="text-[13px] text-[#8A8E99]">
              Showing <strong className="text-[#F0EDE8]">{totalCount}</strong> results for Accessories
            </span>
