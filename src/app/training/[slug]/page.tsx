...
export default function TrainingDetailPage() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    setSending(true);
    try {
      await supabase.from('event_enquiries').insert({
        event_id: event.id,
        event_title: event.title,
        organizer_id: event.organizer_id,
        buyer_name: f.name,
        buyer_email: f.email,
        buyer_phone: f.phone || null,
  ...
  if (loading) return (
    <div className="min-h-screen bg-[#0D0F13] text-[#F0EDE8]"><Navbar />
      <div className="max-w-[1000px] mx-auto px-4 py-24 text-center"><p className="text-[#8A8E99] uppercase tracking-widest text-sm font-bold">Loading...</p></div><Footer /></div>
  );
  ...
  if (notFound || !event) return (
    <div className="min-h-screen bg-[#0D0F13] text-[#F0EDE8]"><Navbar />
      <div className="max-w-[1000px] mx-auto px-4 py-24 text-center">
        ...
        <h1 style={{ fontFamily"'Barlow Condensed', sans-serif" }} className="text-3xl font-black uppercase mb-2">Training day not found</h1>
        <p className="text-[#8A8E99] text-sm mb-6">This course is not listed, or it has already taken place.</p>
        <Link href="/training" className="inline-block bg-[#C9922A] text-black font-black uppercase tracking-widest text-[13px] px-6 py-3 rounded-sm hover:brightness-110">All training days</Link>
      </div><Footer /></div>
  );
          ...
          <div className="lg:col-span-2 flex flex-col gap-6">
            <div>
              <div className="relative h-[280px] md:h-[380px] bg-[#191C23] rounded-sm overflow-hidden mb-3">
                ...
                <div className={`absolute top-4 right-4 text-[10px] font-black uppercase tracking-widest px-3 py-1.5 rounded-sm border ${skillBadge(event.skill_level)}`}>
                  {event.skill_level}
                </div>
              </div>
              {gallery.length > 1 && (
                <div className="flex gap-2 flex-wrap">
                  {galle.map((g, i) => (
                    <button key={i} onClick={() => setActiveImg(g)}
                      className={`w-20 h-16 rounded-sm overflow-hidden border-2 ${activeImg === g ? 'border-[#C9922A]' : 'border-white/10'}`}>
          ...
          <div className="lg:col-span-1">
            <div className="bg-[#13151A] border border-white/5 rounded-sm p-6 lg:sticky lg:top-6">
              ...
              <button onClick={() => setShowModal(true)}
                className="w-full bg-[#C9922A] text-black font-black uppercase tracking-widest text-[14px] py-3.5 rounded-sm hover:brightness-110 transition-all">
                I&apos;m Interested
              </button>
        ...
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4" onClick={() => setShowModal(false)}>
          <div className="bg-[#191C23] border border-white/10 rounded-sm p-6 max-w-md w-full" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-5">
              ...
              <button onClick={() => setShowModal(false)} className="text-[#8A8E99] hover:text-white text-2xl leading-none">×</button>
            </div>
            {sent ? (
              <div className="bg-[#2A9C6E]/10 border border-[#2A9C6E]/30 rounded-sm p-4 text-center">
                <p classNameext-[#2A9C6E] font-bold">Sent. The organizer will be in touch.</p>
              </div>
            ) : (
              <form onSubmit={submit} className="flex flex-col gap-3">
                ...
                <button type="submit" disabled={sending}
                  className="bg-[#C9922A] text-black font-black uppercase tracking-widest text-[13px] py-3 rounded-sm hover:brightness-110 disabled:opacity-40">
