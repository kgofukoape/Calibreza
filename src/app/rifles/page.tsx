...
export default function RiflesPage() {
  ...
  const fetchListings = async () => {
    ...
    try {
      ...
      // Filter by category (rifles)
      query = query.eq('firearm_type', 'rifle');
          ...
          <div className="bg-[#191C23] border border-white/5 rounded-md p-5 flex flex-col gap-6">
            ...
            <div className="flex flex-col gap-3">
              ...
              <div className="flex flex-col gap-2.5 max-h-[300px] overflow-y-auto pr-2 custom-scrollbar">
                {[
                  'Accuracy International', 'Armalite', 'Barrett', 'Benelli', 'Bergara', 
                  'Browning', 'Bushmaster', 'CZ', 'Christensen Arms', 'Daniel Defense', 
                  'Franchi', 'FN Herstal', 'Heckler & Koch', 'Henry', 'Howa', 
                  'IWI', 'Kimber', 'Lithgow', 'Marlin', 'Mossberg', 
                  'Musgrave', 'Norinco', 'Remington', 'Ruger', 'Sako', 
                  'Savage', 'Sig Sauer', 'Smith & Wesson', 'Springfield Armory', 
                  'Steyr', 'Tikka', 'Truvelo', 'Vektor', 'Weatherby', 'Winchester'
                ].map(brand => (
            ...
            <div className="flex flex-col gap-3 border-t border-white/5 pt-5">
              ...
              <div className="flex flex-col gap-2.5 max-h-[250px] overflow-y-auto pr-2 custom-scrollbar">
                {[
                  '.22 LR', '.223 Remington', '.243 Winchester', '.270 Winchester', 
                  '.30-06 Springfield', '.300 Winchester Magnum', '.308 Winchester', 
                  '.338 Lapua Magnum', '.375 H&H Magnum', '.416 Rigby', '.458 Winchester Magnum', 
                  '5.56x45mm NATO', '6.5 Creedmoor', '6.5x55mm Swedish', '7mm Remington Magnum', 
                  '7.62x39mm', '7.62x51mm NATO', '7.62x54mmR', '7x57mm Mauser', 
                  '8x57mm Mauser', '9.3x62mm', '.17 HMR', '.204 Ruger', '.22-250 Remington', 
                  '.220 Swift', '.257 Roberts', '.260 Remington', '6mm Creedmoor', 
                  '7mm-08 Remington', '.280 Remington', '.300 AAC Blackout'
                ].map(calibre => (
        ...
        <div className="flex-1 flex flex-col gap-6">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-[#191C23] border border-white/5 rounded-md p-4">
            <span className="text-[13px] text-[#8A8E99]">
              Showing <strong className="text-[#F0EDE8]">{totalCount}</strong> results for Rifles
            </span>
