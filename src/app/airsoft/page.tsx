...
export default function AirsoftPage() {
  ...
  useEffect(() => {
    fetchListings();
  }, [currentPage, activeBrands, activeTypes, activePowerSources, activeCategories, activeProvinces, activeConditions, activeMinPrice, activeMaxPrice, activeSellerTypes, sortBy]);
  ...
  const fetchListings = async () => {
    ...
    try {
      ...
      if (activeTypes.length > 0) query = query.in('airsoft_type', activeTypes);
      if (activePowerSources.length > 0) query = query.in('power_source', activePowerSources);
      if (activeCategories.length > 0) query = query.in('firearm_type', activeCategories);
      ...
      <div className="bg-[#191C23] border-b border-white/5 pt-8 pb-8 px-6 md:px-8">
        <div className="max-w-[1280px] mx-auto">
          <div className="text-[11px] text-[#8A8E99] tracking-widest uppercase mb-3 flex items-center gap-2">
            ...
            <span>/</span> 
            <span className="text-[#F0EDE8]">Airsoft</span>
          </div>
          <h1 style={{fontFamily:"'Barlow Condensed', sans-serif"}} className="font-extrabold text-4xl md:text-5xl tracking-wide uppercase text-[#F0EDE8]">
            Browse <span className="text-[#C9922A]">Airsoft</span>
          </h1>
        </div>
      </div>
          ...
          <div className="bg-[#191C23] border border-white/5 rounded-md p-5 flex flex-col gap-6">
            ...
            <div className="flex flex-col gap-3">
              ...
              <div className="flex flex-col gap-2.5">
                {['Rifle', 'Pistol', 'SMG', 'Shotgun', 'Sniper', 'Machine Gun'].map(cat => (
                  <label key={cat} className="flex items-center gap-3 cursor-pointer group">
                    ...
                    />
                    <span className="text-[14px] text-[#F0EDE8] group-hover:text-[#C9922A] transition-colors">{cat}</span>
                  </label>
                ))}
              </div>
            </div>
            ...
            <div className="flex flex-col gap-3 border-t border-white/5 pt-5">
              ...
              <div className="flex flex-col gap-2.5">
                {['AEG (Electric)', 'Gas Blowback (GBB)', 'Spring', 'HPA (High Pressure Air)', 'CO2'].map(type => (
            ...
            <div className="flex flex-col gap-3 border-t border-white/5 pt-5">
              ...
              <div className="flex flex-col gap-2.5 max-h-[300px] overflow-y-auto pr-2 custom-scrollbar">
                {[
                  'Tokyo Marui', 'G&G Armament', 'VFC', 'KWA', 'Classic Army', 
                  'ICS', 'CYMA', 'Krytac', 'ASG', 'Elite Force', 
                  'Lancer Tactical', 'G&P', 'Ares', 'WE Tech', 'KJW', 
                  'Umarex', 'Double Eagle', 'Specna Arms', 'Modify', 'Maple Leaf', 
                  'PTS Syndicate', 'EMG', 'Novritsch', 'Arcturus', 'LCT'
                ].map(brand => (
        ...
        <div className="flex-1 flex flex-col gap-6">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-[#191C23] border border-white/5 rounded-md p-4">
            <span className="text-[13px] text-[#8A8E99]">
              Showing <strong className="text-[#F0EDE8]">{totalCount}</strong> results for Airsoft
            </span>
