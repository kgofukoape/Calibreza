...
const PLANS = [
  {
    id: 'free', label: 'Free', price: 0, priceLabel: 'R0', billingLabel: 'forever',
    listingLimit: 5, listingLimitLabel: '5 listings', icon: '🔓',
    features: [
    ...
    color: 'border-white/10', selectedColor: 'border-[#C9922A] bg-[#C9922A]/5',
  },
  {
    id: 'pay_per_ad', label: 'Pay Per Ad', price: 29, priceLabel: 'R29',
    billingLabel: 'per listing', listingLimit: null, listingLimitLabel: '1 per payment', icon: '🎯',
    features: [
      { label: 'Storefront page', included: true },   { label: 'Featured listings', included: true },
      { label: 'Priority support', included: false },
      { label: 'Verified badge', included: false },
      { label: 'No monthly commitment', included: true },
    ],
    color: 'border-white/10', selectedColor: 'border-[#C9922A] bg-[#C9922A]/5', highlight: false,
  },
  {
    id: 'pro', label: 'Pro', price: 499, priceLabel: 'R499', billingLabel: 'per month',
    listingLimit: 50, listingLimitLabel: '50 listings', icon: '⚡',
    features: [
      { label: 'Storefront page', included: true },
    ...
    color: 'border-white/10', selectedColor: 'border-[#C9922A] bg-[#C9922A]/5', highlight: true,
  },
  {
...
export default function SubscriptionPage() {
  ...
  cons[dealer, setDealer] = useState<Dealer | null>(null);
  ...
  useEffect(() => {
    ...
    if (params.get('success') === 'true') setStatusMessage('success');
    else if (params.get('cancel') === 'true') setStatusMessage('cancel');
  }, []);
      ...
      <aside className="w-[280px] bg-[#13151A] border-r border-white/5 flex flex-col">
        ...
        <nav className="flex-1 p-4">
          <ul className="space-y-1">
            <li><Link href="/dealer-dashboard" className="flex items-center gap-3 px-4 py-3 hover:bg-white/5 rounded-sm text-[#8A8E99] hover:text-[#F0EDE8] font-bold text-sm transition-colors"><span>📊</span><span>Overview</span></Link></li>
            <li><Link href="/dealer-dashboard/inventory" className="flex items-center gap-3 px-4 py-3 hover:bg-white/5 rounded-sm text-[#8A8E99] hover:text-[0EDE8] font-bold text-sm transition-colors"><span>📦</span><span>Inventory</span></Link></li>
            ...
            <li><Link href="/dealer-dashboard/subscription" className="flex items-center gap-3 px-4 py-3 bg-[#C9922A]/10 border border-[#C9922A]/20 roundet-[#C9922A] font-bold text-sm"><span>💳</span><span>Subscription</span></Link></li>
        ...
        <div className="p-4 border-t border-white/5 space-y-2">
          ...
          <button onClick={handleLogout} className="w-full text-center px-4 py-2 bg-red-500/10 hover:bg-red-500/20 border bo-500/20 rounded-sm text-red-400 text-sm font-bold transition-colors">Logout</button>
        </div>
      </aside>
      ...
      <main className="flex-1 overflow-y-auto">
        ...
        <div className="p-8 space-y-8">
          ...
          {statusMessage === 'cancel' && (
            <div className="p-4 bg-red-500/10 borderder-red-500/30 rounded-sm text-red-400 font-bold text-sm">
              ❌ Payment cancelled. Your current plan remains unchanged.
            </div>
          )}

          <div className="bg-[#13151A] border border-white/5 rounded-sm p-6">
            <h2 style={{ fontFamily: "'Barlow Condensed', sans-serif" }} className="text-2xl font-black uppercase mb-6">Current <span className="text-[#C9922A]">Plan</span></h2>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="flex flex-col gap-1">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span style={{ fontFamily: "'Barlow Condensed', sans-serif" }} className="text-2xl font-black uppercase text-[#C9922A]">{currentPlan.label}</span>
                </div>
              ...
              <div className="flex flex-col gap-1">
                ...
                <span style={{ fontFamily: "'Barlow Condensed', sans-serif" }} className="text-2xl font-black">
                  {activeListings}<span className="text-[#8A8E99] text-lg"> / {currentLimit === null ? '∞' : currentLimit}</span>
                </span>
                {currentLimit !== null && (
                  <div className="w-full h-1.5 bg-[#0D0F13] rounded-full overflow-hidden mt-1">
                    <div className={`h-full rounded-full transition-all ${activeListings >= currentLimit ? 'bg-red-500' : 'bg-[#C9922A]'}`}
                      style={{ width: `${Math.min((activeListings / currentLimit) * 100, 100)}%` }} />
                  </div>
                )}
              </div>
              <div className="flex flex-col ga1">
          ...
          <div>
            <h2 style={{ fontFamily: "'Barlow Condensed', sans-serif" }} className="text-2xl font-black uppercase mb-6">Change <span className="text-[#C9922A]">Plan</span></h2>
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
              {PLANS.map((plan) => {
                const isCurrent = plan.id === dealer?.subscription_tier;
                const isSected = selectedPlan === plan.id;
                const isPaid = plan.price > 0 && plan.id !== 'pay_per_ad';
                return (
                  <div key={plan.id} className={`relative border-2 rounded-sm p-6 flex flex-col transition-all ${isCurrent ? 'border-[#C9922A] bg-[#C9922A]/5' : isSelected ? 'border-[#C9922A]/60 bg-[#C9922A]/5' : 'border-white/10 hover:border-white/20'}`}>
                    {isCurrent && <div className="absolute top-3 right-3 bg-[#C9922A] text-black text-[9px] font-black uppercase tracking-widest px-2 py-0.5 rounded-sm">Current</div>}
                    {plan.highlight && !isCurrent && <div className="absolute top-3 right-3 bg-white/10 text-[#F0EDE8] text-[9px] font-black uppercase tracking-widest px-2 py-0.5 rounded-sm">Popular</div>}
                    <div className="text-3xl mb-3">{plan.icon}</div>
                    <h3 style={{ fontFamily: "'Barlow Condensed', sans-serif" }} className="text-xl font-black uppercase mb-1">{plan.label}</h3>
                    <span style={{ fontFamily: "'Barlow Condensed', sans-serif" }} className="text-3xl font-black text-[#C9922A] mb-1">{plan.priceLabel}</span>
                    <span className="text-[11px] text-[#8A8E99] mb-4">{plan.billingLabel}</span>
                    <div className="text-[11px] font-black uppercase tracking-widest text-[#8A8E99] mb-3">{plan.listingLimitLabel}</div>
                    <div className="flex flex-col gap-2 mb-6 flex-1">
                      {plan.features.map((f) => (
                        <div key={f.label} className="flex items-center gap-2">
                          <span className={`text-xs flex-shrink-0 ${f.included ? 'text-[#2A9C6E]' : 'text-[#8A8E99]/40'}`}>{f.included ? '✓' : '✗'}</span>
                          <span className={`text-[12px] ${f.included ? 'text-[#F0EDE8]' : 'text-[#8A8E99]/40'}`}>{f.label}</span>
                        </div>
                      ))}
                    </div>
                    <button
                      onClick={() => handleSelectPlan(.id)}
                      disabled={isCurrent || plan.id === 'free' || plan.id === 'pay_per_ad'}
                      className={`w-full py-2.5 rounded-sm text-[11px] font-black uppercase tracking-widest transition-all ${isCurrent ? 'bg-[#C9922A]/20 text-[#C9922A] cursor-not-allowed' : plan.id === 'free' || plan.id === 'pay_per_ad' ? 'bg-white/5 text-[#8A8E99] cursor-not-allowed' : 'bg-white/5 border border-white/10 text-[#F0EDE8] hover:bg-[#C9922A] hover:text-black hover:border-[#C9922A]'}`}
                    >
                      {isCurrent ? 'Current Plan' : isPaid ? 'Subscribe' : 'Contact Us'}
                    </button>
                  </div>
                );
              })}
          ...
          {confirming && selectedPlan && (
            <div className="bg-[#13151A] border border-[#C9922A]/20 rounded-sm p-6">
              <h2 style={{ fontFamily: "'Barlow Condensed', sans-serif" }} className="text-2xl font-black uppercase mb-6">Confirm <span className="text-[#C9922A]">Subscription</span></h2>
              <div className="space-y-3 mb-6">
                ...
                <div className="flex justify-between items-center py-3 border-b border-white/5">
                  ...
                  <span className="text-sm font-bold text-[#C9922A] uppercase">{PLANS.find((p) => p.id === selectedPlan)?.label}</span>
                </div>
                <div className="flex justify-between items-center py-3 border-b border-white/5">
                  ...
                  <span style={{ fontFamily: "'Barlow Condensed', sans-serif" }} className="text-2xl font-black text-[#C9922A]">
                    {PLANS.find((p) => p.id === selectedPlan)?.priceLabel}<span className="text-sm text-[#8A8E99]"> /mo</span>
                  </span>
                </div>
                <div className="flex justify-between items-center py-3 border-b border-white/5">
              ...
              <div className="bg-[#C9922A]/5 border border-[#C9922A]/20 rounded-sm p-4 mb-6">
                <p className="text-[12px] text-[#8A8E99] leading-relaxed">
                  🔒 You will be redirected to PayFast to complete your subscription securely. Your card will be charged <strong className="text-[#F0EDE8]">{PLANS.find((p) => p.id === selectedPlan)?.priceLabel}</strong> today and automatically every month. Cancel anytime by contacting us.
                </p>
              </div>
              <div className="flex items-center gap-4">
                <button onClick={() => { setConfirming(false); setSelectedPlan(null); }} className="px-6 py-3 border border-white/10 rounded-sm text-sm font-black uppercase tracking-widest text-[#8A8E99] hover:bg-white/5 transition-all">Cancel</button>
                <button onClick={handlePayFastSubscription} disabled={redirecting} className="flex-1 bg-[#C9922A] text-black px-8 py-3 rounded-sm font-black uppercase tracking-widest text-[13px] hover:brightness-110 transition-all disabled:opacity-50 disabled:cursor-not-allowed">
                  {redirecting ? 'Redirecting to PayFast...' : '💳 Pay & Subscribe'              </button>
              </div>
            </div>
          )}
          ...
          <div className="bg-[#13151A] border border-white/5 rounded-sm overflow-hidden">
            <div className="p-6 border-b border-white/5">
              <h2 style={{ fontFamily: "'Barlow Condensed', sans-serif" }} className="text-2xl font-black uppercase">Plan <span className="text-[#C9922A]">Comparison</span></h2>
            </div>
              ...
              <table className="w-full">
                  ...
                  <tr className="border-b border-white/5">
                    ...
                    {PLANS.map((plan) => (
                      <th key={plan.id} className="p-4 text-center">
                        <div className={`text-[11px] font-black uppercase tracking-widest ${plan.id === dealer?.subscription_tier ? 'text-[#C9922A]' : 'text-[#F0EDE8]'}`}>{plan.label}</div>
                        {plan.id === dealer?.subscription_tier && <div className="text-[9px] text-[#C9922A] font-bold mt-0.5">Current</div>}
                      </th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  <tr className="border-b border-white/5">
                    ...
                    {PLANS.map((plan) => <td key={plan.id} className="p-4 text-center text-sm font-black text-[#C9922A]">{plan.priceLabel}</td>)}
                  </tr>
                  <tr className="border-b border-white/5">
                    <td className="p-4 text-sm text-[#8A8E99]">Listing Allowance</td>
                    {PLANS.map((plan) => <td key={plan.id} className="p-4 text-center text-sm font-bold text-[#F0EDE8]">{plan.listingLimitLabel}</td>)}
                  </tr>
                  {['Storefront page', 'Featured listings', 'Priority support', 'Verified badge', 'No monthly commitment'].map((feature) => (
                    <tr key={feature} className="border-b border-white/5 last:border-0">
                      ...
                      {PLANS.map((plan) => {
                        ...
                        return <td key={plan.id} className="p-4 text-center"><span className={`text-base ${f?.included ? 'text-[#2A9C6E]' : 'text-[#8A8E99]/30'}`}>{f?.included ? '✓' : '✗'}</span></td>;
                      })}
                    </tr>
                  ))}
                </tbody>
