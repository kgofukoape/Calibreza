...
export default function AmmunitionPage() {
  ...
  const [activeQuantities, setActiveQuantities] = useState<string[]>([]);
  ...
  useEffect(() => {
    fetchListings();
  }, [currentPage, activeBrands, activeCalibres, activeTypes, activeQuantities, activeProvinces, activeConditions, activeMinPrice, activeMaxPrice, activeSellerTypes, sortBy]);
  ...
  const fetchListings = async () => {
    ...
    try {
      ...
      if (activeTypes.length > 0) query = query.in('ammo_type', activeTypes);
      if (activeQuantities.length > 0) query = query.in('quantity', activeQuantities);
  ...
  const applyFilters = () => {
    ...
    setActiveQuantities(selectedQuantities);
          ...
          <div className="bg-[#191C23] border border-white/5 rounded-md p-5 flex flex-col gap-6">
            ...
            <div className="flex flex-col gap-3">
              ...
              <div className="flex flex-col gap-2.5">
                {['FMJ (Full Metal Jacket)', 'HP (Hollow Point)', 'JHP (Jacketed Hollow Point)', 'SP (Soft Point)', 'Match Grade', 'Tracer', 'Frangible', 'Subsonic', 'Buckshot', 'Birdshot', 'Slug'].map(type => (
            ...
            <div className="flex flex-col gap-3 border-t border-white/5 pt-5">
              ...
              <div className="flex flex-col gap-2.5 max-h-[300px] overflow-y-auto pr-2 custom-scrollbar">
                {[
                  '9mm Luger', '.45 ACP', '.40 S&W', '.380 ACP', '10mm Auto', 
                  '.38 Special', '.357 Magnum', '.44 Magnum', '.22 LR', '.22 WMR',
                  '.223 Remington', '5.56x45mm NATO', '.308 Winchester', '7.62x51mm NATO', 
                  '.30-06 Springfield', '.300 Winchester Magnum', '6.5 Creedmoor', 
                  '7.62x39mm', '.270 Winchester', '.243 Winchester', '7mm Remington Magnum',
                  '12 Gauge', '20 Gauge', '16 Gauge', '.410 Bore'
                ].map(calibre => (
            ...
            {/* Quantity Filter */}
            <div className="flex flex-col gap-3 border-t border-white/5 pt-5">
              <span className="text-[12px] font-bold tracking-widest uppercase text-[#8A8E99]">Quantity</span>
              <div className="flex flex-col gap-2.5">
                {['20 Rounds', '25 Rounds', '50 Rounds', '100 Rounds', '250 Rounds', '500 Rounds', '1000 Rounds', 'Bulk (1000+)'].map(qty => (
                  <label key={qty} className="flex items-center gap-3 cursor-pointer group">
                    <input 
                      type="checkbox" 
                      checked={selectedQuantities.includes(qty)}
                      onChange={() => setSelectedQuantities(prev => prev.includes(qty) ? prev.filter(q => q !== qty) : [...prev, qty])}
                      className="w-4 h-4 rounded-sm bg-[#0D0F13] border border-white/10 checked:bg-[#C9922A] checked:border-[#C9922A] appearance-none flex items-center justify-center relative after:content-['✓'] after:absolute after:text-black after:text-[10px] after:opacity-0 checked:after:opacity-100 transition-all flex-shrink-0" 
                    />
                    <span className="text-[14px] text-[#F0EDE8] group-hover:text-[#C9922A] transition-colors">{qty}</span>
                  </label>
                ))}
              </div>
            </div>
            ...
            <div className="flex flex-col gap-3 border-t border-white/5 pt-5">
              ...
              <div className="flex flex-col gap-2.5 max-h-[300px] overflow-y-auto pr-2 custom-scrollbar">
                {[
                  'PMP', 'Hornady', 'Federal', 'Winchester', 'Remington', 
                  'Sellier & Bellot', 'Fiocchi', 'PMC', 'Magtech', 'Blazer', 
                  'Norma', 'Lapua', 'PPU (Prvi Partizan)', 'Wolf', 'Tula', 
                  'Aguila', 'CCI', 'Speer', 'Barnes', 'Nosler'
                ].map(brand => (
        ...
        <div className="flex-1 flex flex-col gap-6">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-[#191C23] border border-white/5 rounded-md p-4">
            <span className="text-[13px] text-[#8A8E99]">
              Showing <strong className="text-[#F0EDE8]">{totalCount}</strong> results for Ammunition
            </span>
