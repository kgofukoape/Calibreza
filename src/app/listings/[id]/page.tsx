        ...
        <div className="flex-1 min-w-0 flex flex-col gap-5">
          ...
          <div className="lg:hidden bg-[#191C23] border border-white/5 rounded-sm p-5">
            ...
            <div style={{ fontFamily: "'Barlow Condensed', sans-serif" }} className="font-extrabold text-4xl text-[#C9922A] leading-none mb-4">
              R {listing.price?.toLocaleString()}{listing.is_negotiable && <span className="text-[16px] text-[#8A8E99] ml-2 font-bold">ONO</span>}
            </div>
          ...
          <div className="bg-[#C9922A]/5 border border-[#C9922A]/20 rounded-sm p-5">
            ...
            <p className="text-[13px] text-[#8A8E99] leading-relaxed mb-2">
              All firearm transactions must comply with the Firearms Control Act (Act 60 of 2000). Buyer must possess a valid firearm licence. Transfer must be completed through a licensed dealer.
            </p>
            <Link href="/dealers" className="text-[#C9922A] text-[12px] font-bold uppercase tracking-widest hover:brightness-125 transition-all">Find a Licensed Dealer Near You →</Link>
          </div>
          ...
          <div className="lg:hidden flex items-center justify-center gap-6 text-[11px] font-bold tracking-widest uppercase text-[#8A8E99]">
            ...
            <span>|</span>
            <button onClick={() => router.push(`/report?listing=${listing.id}&title=${encodeURIComponent(listing.title)}`)} className="hover:text-red-400 transition-colors text-red-500/70">🚩 Report</button>
          </div>
        </div>
        ...
        <aside className="hidden lg:flex w-[340px] xl:w-[380px] flex-shrink-0 flex-col gap-4">
          <div className="bg-[#191C23] border border-white/5 rounded-sm p-6 flex flex-col gap-4">
            ...
            <div style={{ fontFamily: "'Barlow Condensed', sans-serif" }} className="font-extrabold text-[44px] xl:text-[52px] text-[#C9922A] leading-none">
              R {listing.price?.toLocaleString()}{listing.is_negotiable && <span className="text-[18px] text-[#8A8E99] ml-2 font-bold">ONO</span>}
            </div>
            ...
            <div className="flex items-center justify-center gap-4 text-[11px] font-bold tracking-widest uppercase text-[#8A8E99] border-t border-white/5 pt-3">
              ...
              <span>|</span>
              <button onClick={() => router.push(`/report?listing=${listing.id}&title=${encodeURIComponent(listing.title)}`)}
              ...
              {seller?.phone && (
                <a href={`tel:${seller.phone}`} className="flex items-center gap-4 bg-[#0D0F13] border border-white/10 rounded-sm px-4 py-3.5 hover:border-[#C9922A]/50 transition-all group">
                  <div className="w-9 h-9 bg-[#C9922A]/10 rounded-full flex items-center justify-center flex-shrink-0">
                    <svg className="w-4 h-4 text-[#C9922A]" fill="currentColor" viewBox="0 0 20 20"><path d="M2 3a1 1 0 011-1h2.153a1 1 0 01.986.836l.74 4.435a1 1 0 01-.54 1.06l-1.548.773a11.037 11.037 0 006.105 6.105l.774-1.548a1 1 0 011.059-.54l4.435.74a1 1 0 01.836.986V17a1 1 0 01-1 1h-2C7.82 18 2 12.18 2 5V3z"/></svg>
                  </div>
                  <div>
                  ...
                  <button
                    onClick={async () => {
                      ...
                      const { error } = await supabase.from('user_messages').insert({
                        ...
                        listing_id: listing.id, body: messageBody.trim(), is_read: false,
                      });
