export default function ListingCard({ id, title, make, price, province, condition, listingType, sellerName, featured, calibre }: any) {
  return (
    <a href={`/listings/${id}`} style={{background:'#191C23',border:'1px solid rgba(255,255,255,0.06)',borderRadius:4,overflow:'hidden',textDecoration:'none',display:'flex',flexDirection:'column',color:'#F0EDE8'}}>
      <div style={{position:'relative',aspectRatio:'4/3',background:'#1F2330',display:'flex',alignItems:'center',justifyContent:'center'}}>
        ...
        <div style={{position:'absolute',top:10,left:10,background:listingType==='dealer'?'rgba(42,156,110,0.12)':'rgba(74,127,193,0.12)',border:listingType==='dealer'?'1px solid rgba(42,156,110,0.3)':'1px solid rgba(74,127,193,0.3)',color:listingType==='dealer'?'#2A9C6E':'#4A7FC1',fontSize:10,fontWeight:700,textTransform:'uppercase',padding:'3px 8px',borderRadius:2}}>
      ...
      <div style={{padding:'14px 16px',flex:1,display:'flex',flexDirection:'column',gap:6}}>
        <div style={{fontSize:11,color:'#C9922A',fontWeight:600,textTransform:'uppercase'}}>{make}</div>
